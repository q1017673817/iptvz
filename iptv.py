...
print("开始获取酒店源")
file_path = f"ip/酒店高清.ip"
print(f"开始测试 '{file_path}'")
results = sorted(set(scan_ip_port(file_path)))   
    ...
    while True:
        ...
        try:
            ...
            if content:
                ...
                print(f"总频道：{len(channels)}，可用：{len(results)}个，不可用：{len(error_channels)}个，进度：{numberx:.2f} %。")

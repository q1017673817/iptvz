...
case $city_choice in
    1)
        ...
        channel_key="江苏"
        url_fofa="https://www.zoomeye.org/searchResult?q=%22udpxy%22%20%2Bsubdivisions%3A%22jiangsu%22%20%2Borg%3A%20%22CHINANET%22%20%2Bservice%3A%22http%22"
        ;;
    2)
        city="Hubei_90"
        stream="rtp/239.254.96.96:8550"
        channel_key="湖北电信"
        url_fofa="https://www.zoomeye.org/searchResult?q=%22udpxy%22%20%2Bsubdivisions%3A%22hubei%22%20%2Borg%3A%20%22CHINANET%22%20%2Bservice%3A%22http%22"
        ;;
    3)
        city="Shanghai_103"
...
echo "$ipfile"
grep -E -o '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+:[0-9]+' test.html | sort | uniq > "$ipfile"

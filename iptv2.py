...
with open('itv0.txt', 'r', encoding='utf-8') as file:
#从整理好的文本中按类别进行特定关键词提取
 keywords = ['tsfile']  # 需要提取的关键字列表
 pattern = '|'.join(keywords)  # 创建正则表达式模式，匹配任意一个关键字
#pattern = r"^(.*?),(?!#genre#)(.*?)$" #以分类直接复制
with open('itv0.txt', 'r', encoding='utf-8') as file, open('itv1.txt', 'w', encoding='utf-8') as a:    #####定义临时文件名
    a.write('\n')                                                                  #####写入临时文件名
    for line in file:
...
############
file_contents = []
file_paths = ["a2.txt","b2.txt","zubo.txt","c2.txt","d2.txt","e2.txt","f2.txt","g2.txt","h2.txt","i2.txt","j2.txt","k2.txt","l2.txt","m2.txt","n2.txt","o2.txt","p2.txt","q2.txt","r2.txt","s2.txt","t2.txt","z2.txt"]  # 替换为实际的文件路径列表
...
# 写入合并后的文件
with open("去重2.txt", "w", encoding="utf-8") as output:
    output.write('\n'.join(file_contents))

# 原始顺序去重
# 打开文档并读取所有行 
with open('去重2.txt', 'r', encoding="utf-8") as file:
 lines = file.readlines()
